// ISO 3166-1 alpha-2 → alpha-3 mapping. Open-Meteo geocoder returns alpha-2
// (e.g. "IN", "US", "JP"); OWID's `iso_code` column uses alpha-3 ("IND",
// "USA", "JPN"). We index emissions_annual by both name and iso_code so the
// compute layer can prefer the iso path and bypass country-name aliasing.
//
// Source: ISO 3166-1. Includes commonly-occurring territories (HK, MO, TW)
// that visitors might enter.
...
export function alpha2ToAlpha3(code: string | null | undefined): string | null {
  if (!code) return null;
